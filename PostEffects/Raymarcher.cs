...
namespace CM3D2.SceneCapture.Plugin
{
    ...
    public class Raymarcher : BaseEffect
    {
        // public Material m_material;
        public bool screenSpace = true;
        public bool enableAdaptive = true;
        public bool enableTemporal = true;
        public bool enableGlowline = true;
        public bool dbgShowSteps;
        public int scene;
        public Color fogColor = new Color(0.16f, 0.13f, 0.20f);
        ...
        void Awake()
                  {
                      ...
                      var cam = GetComponent<Camera>();
                                              ...
                                              m_internal_material = new Material(Material);
                                              var r = GetComponent<Renderer>();
                                              if (r != null) { r.sharedMaterial = m_internal_material; }


                                              m_enable_adaptive_prev = enableAdaptive;
                  }
        ...
        void UpdateMaterial()
        {
            ...
            m_internal_material.SetFloat("_Scene", scene);
            SwitchKeyword(m_internal_material, "ENABLE_ADAPTIVE",    enableAdaptive );
            SwitchKeyword(m_internal_material, "ENABLE_TEMPORAL",    enableTemporal );
            SwitchKeyword(m_internal_material, "ENABLE_PATTERN",     enableGlowline );
            SwitchKeyword(m_internal_material, "ENABLE_SCREENSPACE", screenSpace    );
            if (!screenSpace)
            {
        ...
        void UpdateCommandBuffer()
        {
            ...
            if (m_enable_adaptive_prev != enableAdaptive)
            {
                m_enable_adaptive_prev = enableAdaptive;
                reflesh_command_buffer = true;
            }
            if (m_dbg_show_steps_prev != dbgShowSteps)
            {
                m_dbg_show_steps_prev = dbgShowSteps;
                reflesh_command_buffer = true;
            }
